...
const ProductCard = memo(function ProductCard({
  ...
  return (
    <article
      ...
      className="rd-card-in group flex flex-col overflow-hidden rounded-2xl border border-[color:var(--rd-ink)]/10 bg-[color:var(--rd-paper-bright)] rd-shadow-luxe transition-[transform,border-color,box-shadow] duration-500 [transition-timing-function:var(--ease-out)] hover:-translate-y-1 hover:border-[color:var(--rd-amber-dark)]/45 hover:shadow-[0_30px_70px_rgba(27,58,44,0.13)]"
    >
      ...
      <div className="flex flex-1 flex-col p-5">
          ...
          */}
          <div className="shrink-0 [font-family:var(--font-mono)]">
            {product.variants.length > 1 ? (
              <div className="grid grid-cols-[auto_auto] items-baseline gap-x-3 gap-y-1 text-right">
                {product.variants.map((variant, i) => (
                  <Fragment key={variant.label}>
                    ...
                    <span className={`font-semibold tabular-nums text-[color:var(--rd-amber-dark)] ${i === 0 ? 'text-xl sm:text-2xl' : 'text-base sm:text-lg opacity-85'}`}>
              ...
              <p className="text-xl font-semibold text-[color:var(--rd-amber-dark)] sm:text-2xl text-right">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {product.weight && product.variants.length <= 1 && <span className="rounded-full border border-[color:var(--rd-ink)]/12 bg-[color:var(--rd-paper-bright)] px-3 py-1 text-[11px] tracking-wider text-[color:var(--rd-on-paper-dim)] [font-family:var(--font-mono)]">{product.weight}</span>}
          <span className="rounded-full border border-[color:var(--rd-ink)]/12 bg-[color:var(--rd-paper-bright)] px-3 py-1 text-[11px] tracking-wider text-[color:var(--rd-on-paper-dim)] [font-family:var(--font-mono)]">{inferProfile(product)}</span>
          {potency && <span className="rounded-full border border-[color:var(--rd-ink)]/12 bg-[color:var(--rd-glow)]/10 px-3 py-1 text-[11px] tracking-wider text-[color:var(--rd-moss)] [font-family:var(--font-mono)]">{potency}</span>}
        </div>

        {getProductDescription(product) && (
          <p className="product-description-clamp mt-4 text-sm leading-7 text-[color:var(--rd-on-paper-dim)]">{getProductDescription(product)}</p>
        )}

        <div className="mt-auto flex items-center justify-between gap-3 pt-6">
          <button onClick={() => onDetails(product)} className="btn-luxe btn-luxe-outline btn-luxe-sm">
            Details
          </button>
          <Link
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="max-h-[100dvh] w-full max-w-5xl overflow-y-auto border border-[color:var(--rd-ink)]/10 bg-[color:var(--rd-paper-bright)] text-[color:var(--rd-ink)] rd-shadow-luxe outline-none sm:max-h-[92vh] sm:rounded-3xl"
          ...
          <div className="p-6 sm:p-8 md:p-10">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <p className="rd-eyebrow text-[color:var(--rd-moss)]">{product.category}</p>
                <h2
                ...
                <p className="mt-2 text-sm text-[color:var(--rd-on-paper-mute)]">{getBrandLabel(product)}</p>
              </div>
              <button
                onClick={onClose}
              ...
              {product.variants.length > 1 ? (
                <div className="flex flex-wrap items-end gap-x-6 gap-y-3">
                  {product.variants.map((variant, i) => (
                    <div key={variant.label} className="flex flex-col">
                      <p className={`font-semibold text-[color:var(--rd-amber-dark)] ${i === 0 ? 'text-4xl sm:text-5xl' : 'text-2xl sm:text-3xl opacity-80'}`}>
                        {formatPrice(variant.price)}
                      </p>
                      <p className="mt-1 text-[11px] uppercase tracking-[0.18em] text-[color:var(--rd-on-paper-mute)]">
                        {variant.label}
                      </p>
                    </div>
                  ))}
                  <p className="ml-auto pb-1 text-[11px] uppercase tracking-[0.16em] text-[color:var(--rd-on-paper-mute)]">
                ...
                <div className="flex items-end gap-3">
                  {product.salePrice < product.price && (
                    <p className="pb-1 text-sm font-medium text-[color:var(--rd-on-paper-mute)] line-through">{formatPrice(product.price)}</p>
                  )}
                  <p className="text-4xl font-semibold text-[color:var(--rd-amber-dark)] sm:text-5xl">{formatPrice(product.salePrice)}</p>
                </div>
              )}
            </div>

            {getProductDescription(product) && (
              <p className="mt-5 text-base leading-8 text-[color:var(--rd-on-paper-dim)]">{getProductDescription(product)}</p>
            )}

            {effects.length > 0 && (
              <div className="mt-6 flex flex-wrap items-center gap-2">
                <span className="rd-eyebrow text-[color:var(--rd-on-paper-mute)]">Reported effects</span>
                {effects.map((effect) => (
                  <span
                    key={effect}
                    className="inline-flex items-center gap-1 rounded-full border border-[color:var(--rd-ink)]/12 bg-[color:var(--rd-paper-bright)] px-3 py-1 text-xs text-[color:var(--rd-on-paper-dim)] [font-family:var(--font-mono)]"
                  >
                    <Sparkles className="h-3 w-3 text-[color:var(--rd-moss)]" />
                    {effect}
                  </span>
                ))}
              </div>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              {detailRows.map(([label, value]) => (
                <div
                  key={label}
                  className="rounded-2xl border border-[color:var(--rd-ink)]/10 bg-[color:var(--rd-paper-bright)] p-4"
                >
                  <p className="rd-eyebrow text-[color:var(--rd-on-paper-mute)]">{label}</p>
                  <p className="mt-2 text-sm font-medium text-[color:var(--rd-ink)]">{value}</p>
                </div>
              ))}
            </div>

            {product.deals.length > 0 && (
              <div className="mt-6 rounded-2xl border border-[color:var(--rd-amber-dark)]/40 bg-[color:var(--rd-amber)]/10 p-4">
                <p className="rd-eyebrow text-[color:var(--rd-amber-dark)]">Active deal</p>
                <p className="mt-2 text-sm font-medium text-[color:var(--rd-ink)]">{product.deals[0].name}</p>
              </div>
            )}
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              ...
              <button onClick={share} className="btn-luxe btn-luxe-outline">
                <Share2 className="h-4 w-4" />
                {copied ? 'Link copied' : 'Share'}
              </button>
              <button onClick={onClose} className="btn-luxe btn-luxe-outline">
                Keep browsing
              </button>
            </div>
            ...
            <p className="mt-4 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-[color:var(--rd-on-paper-dim)]">
              <span className="inline-flex items-center gap-1.5 font-medium text-[color:var(--rd-moss)]">
                <Check className="h-3.5 w-3.5" />
                Free pre-roll gift added
              </span>
              <span aria-hidden className="text-[color:var(--rd-ink)]/25">·</span>
...
function SelectField({ label, value, onChange, children }: { label: string; value: string; onChange: (value: string) => void; children: React.ReactNode }) {
  return (
    <label className="grid gap-2">
      <span className="rd-eyebrow text-[color:var(--rd-on-paper-mute)]">{label}</span>
      <select
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="h-12 rounded-full border border-[color:var(--rd-ink)]/12 bg-[color:var(--rd-paper-bright)] px-4 text-sm font-medium text-[color:var(--rd-ink)] outline-none transition hover:border-[color:var(--rd-amber-dark)]/45 focus:border-[color:var(--rd-moss)] focus:shadow-[0_0_0_4px_rgba(198,160,100,0.18)] [font-family:var(--font-sans)]"
      >
        {children}
      </select>
    </label>
...
export default function MenuExplorer({ initialCategory, initialProductId, initialDealsOnly, initialEffect }: { initialCategory?: string; initialProductId?: string; initialDealsOnly?: boolean; initialEffect?: string }) {
  ...
  return (
    <SiteChrome>
        ...
        <div className="luxury-shell relative py-12 sm:py-16 lg:py-20">
          ...
          */}
          <div className="mb-8 flex justify-center sm:mb-10">
            <div className="relative h-28 w-28 overflow-hidden rounded-full border border-[color:var(--rd-ink)]/12 bg-[color:var(--rd-paper-bright)] rd-shadow-luxe sm:h-32 sm:w-32">
              <BrandLogoLoop />
            </div>
          </div>
          ...
          <div>
            <Breadcrumbs items={[{ label: 'Menu' }]} tone="light" />
            <p className="mt-5 rd-eyebrow text-[color:var(--rd-moss)]">Raindrops NY menu</p>
            ...
            <p className="mt-5 inline-flex items-start gap-2 rounded-2xl border border-[color:var(--rd-ink)]/12 bg-[color:var(--rd-glow)]/10 px-4 py-2 text-sm font-medium text-[color:var(--rd-ink)]">
              <Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-[color:var(--rd-moss)]" />
              ...
              return (
                <button
                  ...
                  className={`group rounded-2xl border p-3 text-left transition-[transform,border-color,background-color,box-shadow] duration-500 [transition-timing-function:var(--ease-out)] hover:-translate-y-0.5 sm:p-4 ${
                    active
                      ? 'border-[color:var(--rd-amber-dark)]/45 bg-[color:var(--rd-glow)]/10 rd-shadow-luxe'
                      : 'border-[color:var(--rd-ink)]/10 bg-[color:var(--rd-paper-bright)] hover:border-[color:var(--rd-amber-dark)]/45 hover:bg-[color:var(--rd-paper-bright)]'
                  }`}
                >
                  {(() => {
                    ...
                    return (
                      <Icon
                        className={`h-5 w-5 transition-colors ${active ? 'text-[color:var(--rd-moss)]' : 'text-[color:var(--rd-on-paper-dim)] group-hover:text-[color:var(--rd-moss)]'}`}
                        strokeWidth={1.6}
                      />
                    );
                  })()}
                  <p className={`mt-2 rd-eyebrow ${active ? 'text-[color:var(--rd-moss)]' : 'text-[color:var(--rd-on-paper-mute)]'}`}>{CATEGORY_LABEL[item]}</p>
                  <p
                    className="mt-1 text-[color:var(--rd-ink)]"
      ...
      <section className="border-t border-[color:var(--rd-ink)]/10 bg-[color:var(--rd-paper)] py-8 text-[color:var(--rd-ink)] sm:py-10">
        <div className="luxury-shell">
          <div className="rounded-3xl border border-[color:var(--rd-ink)]/10 bg-[color:var(--rd-paper-bright)] p-5 rd-shadow-luxe sm:p-6">
            <div className="grid gap-4 xl:grid-cols-[1fr_auto] xl:items-center">
              <label className="flex min-w-0 items-center gap-3 rounded-full border border-[color:var(--rd-ink)]/12 bg-[color:var(--rd-paper-bright)] px-5 py-3 transition focus-within:border-[color:var(--rd-moss)]">
                <Search className="h-5 w-5 shrink-0 text-[color:var(--rd-moss)]" />
                <input
                  value={query}
                  onChange={(event) => setFilter(setQuery, event.target.value)}
                  className="min-w-0 flex-1 bg-transparent text-sm text-[color:var(--rd-ink)] outline-none placeholder:text-[color:var(--rd-on-paper-mute)]"
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-full px-4 py-2.5 text-[11px] font-semibold uppercase tracking-[0.16em] transition [font-family:var(--font-mono)] ${
                        active
                          ...
                          : 'border border-[color:var(--rd-ink)]/12 bg-[color:var(--rd-paper-bright)] text-[color:var(--rd-on-paper-dim)] hover:border-[color:var(--rd-amber-dark)]/45 hover:text-[color:var(--rd-ink)]'
                      }`}
                    >
            ...
            <div className={`mt-5 grid gap-4 ${cols}`}>
              ...
              <label className="grid gap-2">
                <span className="rd-eyebrow text-[color:var(--rd-on-paper-mute)]">
                  Max price <span className="text-[color:var(--rd-moss)]">{formatPrice(priceMax * 100)}</span>
                </span>
                <input
                  type="range"
                  min={15}
              ...
              {showMinThc && (
                <label className="grid gap-2">
                  <span className="rd-eyebrow text-[color:var(--rd-on-paper-mute)]">
                    Min THC <span className="text-[color:var(--rd-moss)]">{minThc}%</span>
                  </span>
                  <input
                    type="range"
                    min={0}
            ...
            <div className="mt-5 flex flex-col gap-3 border-t border-[color:var(--rd-ink)]/10 pt-5 md:flex-row md:items-center md:justify-between">
              ...
              <p className="inline-flex items-center gap-2 text-sm text-[color:var(--rd-on-paper-dim)]">
                <Filter className="h-4 w-4 text-[color:var(--rd-moss)]" />
                Showing <span className="text-[color:var(--rd-ink)] [font-family:var(--font-mono)]">{filteredProducts.length}</span> of <span className="text-[color:var(--rd-ink)] [font-family:var(--font-mono)]">{menuProducts.length}</span> products
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="bg-[color:var(--rd-paper)] pb-20 text-[color:var(--rd-ink)]">
        <div className="luxury-shell">
          ...
          {visibleProducts.length > 0 ? (
            ...
            <div className="rd-card-in mx-auto max-w-2xl rounded-3xl border border-[color:var(--rd-ink)]/10 bg-[color:var(--rd-paper-bright)] p-10 text-center">
              <SlidersHorizontal className="mx-auto h-10 w-10 text-[color:var(--rd-moss)]" />
              <h2 className="mt-5 text-[color:var(--rd-ink)]">
                No products <span className="italic">matched.</span>
              </h2>
              <p className="mt-3 text-[color:var(--rd-on-paper-dim)]">Adjust the filters or reset to see the full menu.</p>
            </div>
          )}

          <div className="mt-10 flex flex-col items-center justify-between gap-4 rounded-3xl border border-[color:var(--rd-ink)]/10 bg-[color:var(--rd-paper-bright)] p-5 md:flex-row">
            <p className="inline-flex items-center gap-2 text-sm text-[color:var(--rd-on-paper-dim)]">
              <Check className="h-4 w-4 text-[color:var(--rd-moss)]" />
              <span className="text-[color:var(--rd-ink)] [font-family:var(--font-mono)]">{visibleProducts.length}</span> visible now
            </p>
